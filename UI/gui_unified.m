function gui_unified()
    ...
    % Modern Color Palette (Dark Theme) - Improved contrast
    bgColor = [0.12 0.12 0.12];           % Darker background
    panelColor = [0.18 0.18 0.18];       % Slightly lighter panels
    textColor = [0.95 0.95 0.95];        % Brighter text
    btnBlue = [0.1 0.45 0.75];           % Brighter blue
    btnGreen = [0.2 0.65 0.3];           % Brighter green
    btnRed = [0.8 0.2 0.2];             % Brighter red
    btnGray = [0.45 0.45 0.45];          % Gray for secondary buttons
    ...
    statusBar = uicontrol(fig, 'Style', 'text', ...
        'Units','normalized', 'Position', [0.0 0.0 1 0.035], ...
        'String', ' Ready', 'HorizontalAlignment', 'left', ...
        'BackgroundColor', [0.08 0.08 0.08], 'ForegroundColor', [1 1 1], 'FontSize', 10);

    % Image panels (Left Side - 2x2 grid)
    % Top row: Original and Processed images side by side
    originalImgPanel = uipanel(fig, 'Title', 'Original Image', ...
        'Units','normalized', 'Position', [0.015 0.54 0.47 0.43], ...
        'BackgroundColor', panelColor, 'ForegroundColor', textColor, 'FontSize', 11, 'BorderWidth', 1);

    axOriginal = axes('Parent', originalImgPanel, 'Units','normalized', 'Position', [0.02 0.02 0.96 0.96]);
    ...
    processedImgPanel = uipanel(fig, 'Title', 'Processed Image', ...
        'Units','normalized', 'Position', [0.515 0.54 0.47 0.43], ...
        'BackgroundColor', panelColor, 'ForegroundColor', textColor, 'FontSize', 11, 'BorderWidth', 1);

    axProcessed = axes('Parent', processedImgPanel, 'Units','normalized', 'Position', [0.02 0.02 0.96 0.96]);
    ...
    controlPanel = uipanel(fig, 'Title', 'Controls', ...
        'Units','normalized', 'Position', [0.015 0.06 0.47 0.45], ...
        'BackgroundColor', panelColor, 'ForegroundColor', textColor, 'FontSize', 11, 'BorderWidth', 1);
    ...
    filterDropdown = uicontrol(controlPanel, 'Style', 'popupmenu', ...
        'String', filterList, 'Units','normalized', 'Position', [0.1 0.80 0.8 0.07], ...
        'Callback', @filterSelected, 'BackgroundColor', [0.25 0.25 0.25], 'ForegroundColor', 'white');

    % Parameter panel (inside control panel, below filter dropdown)
    paramPanel = uipanel(controlPanel, 'Title', 'Filter Parameters', ...
        'Units','normalized', 'Position', [0.05 0.25 0.9 0.50], ...
        'BackgroundColor', [0.22 0.22 0.22], 'ForegroundColor', textColor, 'Visible', 'off', 'BorderWidth', 1);

    % Parameter controls (created dynamically based on filter)
    paramControls = struct();

    % Action buttons (at bottom of control panel)
    uicontrol(controlPanel, 'Style', 'pushbutton', 'String', 'Apply Filter', ...
        'Units','normalized', 'Position', [0.1 0.12 0.36 0.08], ...
    ...
    uicontrol(controlPanel, 'Style', 'pushbutton', 'String', 'Reset Image', ...
        'Units','normalized', 'Position', [0.54 0.12 0.36 0.08], ...
        'Callback', @resetImage, 'BackgroundColor', btnGray, 'ForegroundColor', 'white', 'Enable', 'off');

    uicontrol(controlPanel, 'Style', 'pushbutton', 'String', 'Save Image', ...
        'Units','normalized', 'Position', [0.1 0.03 0.8 0.07], ...
    ...
    function loadImage(~, ~)
        ...
        if filename ~= 0
            ...
            updateStatus('Image loaded successfully. Select a filter and click Apply.');
        end
    end
    ...
    function filterSelected(~, ~)
        ...
        if idx == 1
            ...
            currentFilter = '';
            updateStatus('Select a filter from the dropdown.');
            return;
        end
        ...
        switch currentFilter
            case 'Adjust Brightness'
                uicontrol('Parent', paramPanel, 'Style', 'text', 'String', 'Brightness Delta:', ...
                    'Units','normalized', 'Position', [0.05 0.7 0.4 0.15], ...
                    'HorizontalAlignment', 'left', 'BackgroundColor', [0.22 0.22 0.22], 'ForegroundColor', textColor);
                paramControls.delta = uicontrol('Parent', paramPanel, 'Style', 'edit', ...
                    'String', '50', 'Units','normalized', 'Position', [0.5 0.7 0.45 0.15], ...
            ...
            case 'Butterworth Filter'
                uicontrol('Parent', paramPanel, 'Style', 'text', 'String', 'Cutoff Frequency (D0):', ...
                    'Units','normalized', 'Position', [0.05 0.7 0.4 0.12], ...
                    'HorizontalAlignment', 'left', 'BackgroundColor', [0.22 0.22 0.22], 'ForegroundColor', textColor);
                paramControls.D0 = uicontrol('Parent', paramPanel, 'Style', 'edit', ...
                    'String', '30', 'Units','normalized', 'Position', [0.5 0.7 0.45 0.12], ...
                    'BackgroundColor', [0.3 0.3 0.3], 'ForegroundColor', 'white');
                
                uicontrol('Parent', paramPanel, 'Style', 'text', 'String', 'Order (n):', ...
                    'Units','normalized', 'Position', [0.05 0.5 0.4 0.12], ...
                    'HorizontalAlignment', 'left', 'BackgroundColor', [0.22 0.22 0.22], 'ForegroundColor', textColor);
                paramControls.n = uicontrol('Parent', paramPanel, 'Style', 'edit', ...
                    'String', '2', 'Units','normalized', 'Position', [0.5 0.5 0.45 0.12], ...
                    'BackgroundColor', [0.3 0.3 0.3], 'ForegroundColor', 'white');
                
                uicontrol('Parent', paramPanel, 'Style', 'text', 'String', 'Filter Type:', ...
                    'Units','normalized', 'Position', [0.05 0.3 0.4 0.12], ...
                    'HorizontalAlignment', 'left', 'BackgroundColor', [0.22 0.22 0.22], 'ForegroundColor', textColor);
                paramControls.index = uicontrol('Parent', paramPanel, 'Style', 'popupmenu', ...
                    'String', {'Low Pass', 'High Pass'}, 'Units','normalized', 'Position', [0.5 0.3 0.45 0.12], ...
                    'BackgroundColor', [0.3 0.3 0.3], 'ForegroundColor', 'white');
            ...
            case 'Correlation'
                ...
                paramControls.kernelType = uicontrol('Parent', paramPanel, 'Style', 'popupmenu', ...
                    ...
                    'Units','normalized', 'Position', [0.5 0.6 0.45 0.15], ...
                    'BackgroundColor', [0.3 0.3 0.3], 'ForegroundColor', 'white');
            ...
            case 'Exponential Noise'
                ...
                paramControls.a = uicontrol('Parent', paramPanel, 'Style', 'edit', ...
                    'String', '0.1', 'Units','normalized', 'Position', [0.5 0.7 0.45 0.12], ...
                    'BackgroundColor', [0.3 0.3 0.3], 'ForegroundColor', 'white');
                ...
                paramControls.b = uicontrol('Parent', paramPanel, 'Style', 'edit', ...
                    'String', '1', 'Units','normalized', 'Position', [0.5 0.5 0.45 0.12], ...
                    'BackgroundColor', [0.3 0.3 0.3], 'ForegroundColor', 'white');
            ...
            case 'Gaussian Filter'
                ...
                uicontrol('Parent', paramPanel, 'Style', 'text', 'String', 'Filter Type:', ...
                    ...
                    'HorizontalAlignment', 'left', 'BackgroundColor', [0.22 0.22 0.22], 'ForegroundColor', textColor);
                paramControls.index = uicontrol('Parent', paramPanel, 'Style', 'popupmenu', ...
                    'String', {'Low Pass', 'High Pass'}, 'Units','normalized', 'Position', [0.5 0.5 0.45 0.12], ...
                    'BackgroundColor', [0.3 0.3 0.3], 'ForegroundColor', 'white');
            ...
            case 'Gamma Noise'
                uicontrol('Parent', paramPanel, 'Style', 'text', 'String', 'Parameter a:', ...
                    ...
                    'HorizontalAlignment', 'left', 'BackgroundColor', [0.22 0.22 0.22], 'ForegroundColor', textColor);
                paramControls.a = uicontrol('Parent', paramPanel, 'Style', 'edit', ...
                    'String', '2', 'Units','normalized', 'Position', [0.5 0.7 0.45 0.12], ...
                    'BackgroundColor', [0.3 0.3 0.3], 'ForegroundColor', 'white');
            ...
            case 'Gray to Binary'
                uicontrol('Parent', paramPanel, 'Style', 'text', 'String', 'Threshold Method:', ...
                    ...
                    'HorizontalAlignment', 'left', 'BackgroundColor', [0.22 0.22 0.22], 'ForegroundColor', textColor);
                paramControls.index = uicontrol('Parent', paramPanel, 'Style', 'popupmenu', ...
                    'String', {'Fixed (128)', 'Mean', 'Median'}, 'Units','normalized', 'Position', [0.5 0.6 0.45 0.15], ...
                    'BackgroundColor', [0.3 0.3 0.3], 'ForegroundColor', 'white');
            ...
            case 'Gaussian Noise'
                ...
                paramControls.v = uicontrol('Parent', paramPanel, 'Style', 'edit', ...
                    'String', '20', 'Units','normalized', 'Position', [0.5 0.7 0.45 0.12], ...
                    'BackgroundColor', [0.3 0.3 0.3], 'ForegroundColor', 'white');
                ...
                paramControls.m = uicontrol('Parent', paramPanel, 'Style', 'edit', ...
                    'String', '0', 'Units','normalized', 'Position', [0.5 0.5 0.45 0.12], ...
                    'BackgroundColor', [0.3 0.3 0.3], 'ForegroundColor', 'white');
            ...
            case 'Line Detection'
                ...
                paramControls.direction = uicontrol('Parent', paramPanel, 'Style', 'popupmenu', ...
                    ...
                    'Units','normalized', 'Position', [0.5 0.6 0.45 0.18], ...
                    'BackgroundColor', [0.3 0.3 0.3], 'ForegroundColor', 'white');
            ...
            case 'Salt & Pepper Noise'
                ...
                paramControls.ps = uicontrol('Parent', paramPanel, 'Style', 'edit', ...
                    'String', '0.01', 'Units','normalized', 'Position', [0.5 0.7 0.45 0.12], ...
                    'BackgroundColor', [0.3 0.3 0.3], 'ForegroundColor', 'white');
                ...
                paramControls.pp = uicontrol('Parent', paramPanel, 'Style', 'edit', ...
                    'String', '0.01', 'Units','normalized', 'Position', [0.5 0.5 0.45 0.12], ...
                    'BackgroundColor', [0.3 0.3 0.3], 'ForegroundColor', 'white');
            ...
            case 'Sobel Mask'
                uicontrol('Parent', paramPanel, 'Style', 'text', 'String', 'Direction:', ...
                    ...
                    'HorizontalAlignment', 'left', 'BackgroundColor', [0.22 0.22 0.22], 'ForegroundColor', textColor);
                paramControls.direction = uicontrol('Parent', paramPanel, 'Style', 'popupmenu', ...
                    'String', {'Horizontal', 'Vertical'}, 'Units','normalized', 'Position', [0.5 0.6 0.45 0.15], ...
                    'BackgroundColor', [0.3 0.3 0.3], 'ForegroundColor', 'white');
                
                % For filters with no parameters, just show a message
            case {'Negative', 'Max Filter', 'Mean Filter', 'Median Filter', ...
                 'Midpoint Filter', 'Min Filter', 'Point Sharpening', ...
                 'Point Detection', 'Fourier Transform', 'Inverse Fourier', ...
                 'Histogram Equalization', ...
                 'Histogram', 'Weighted Filter', 'Roberts Mask', ...
                 'Average Filter', 'Contrast Stretch'}
                uicontrol('Parent', paramPanel, 'Style', 'text', 'String', 'No parameters needed for this filter.', ...
                    'Units','normalized', 'Position', [0.1 0.4 0.8 0.2], ...
                    'HorizontalAlignment', 'center', 'BackgroundColor', [0.22 0.22 0.22], 'ForegroundColor', textColor);
        end
        
        paramPanel.Visible = 'on';
        updateStatus(['Selected: ' currentFilter]);
    end

    function applyFilter(~, ~)
        if isempty(currentImg)
            updateStatus('Error: No image loaded. Please load an image first.');
            return;
        end
        
        if strcmp(currentFilter, '') || strcmp(currentFilter, 'Select a filter...')
            updateStatus('Error: No filter selected. Please select a filter from the dropdown.');
            return;
        end
